...
def graficar_QRS(fila, n):
 ...
 #QS
    q_points = []
    s_points = []
    window = int(0.06 *len(fila.values))
    ...
    # max_idx = np.argmax(y)
    # axs[1].scatter([max_idx], [y[max_idx]], color='red', marker='o', label='R')
    # axs[1].text(max_idx, y[max_idx], 'R', fontsize=10, color='red', va='bottom')


    # Graficar puntos Q en verde
    if q_points:
        axs[1].scatter(q_points, y[q_points], color='green', marker='x', s=80, label='Q')
        for idx in q_points:
            axs[1].text(idx, y[idx], 'Q', fontsize=10, color='green', va='bottom')
